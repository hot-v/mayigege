...
public class SidWebSessionManager extends DefaultWebSessionManager {
	...
	public Serializable getSessionId(SessionKey key) {
		...
		if (id == null && WebUtils.isWeb(key)) {
			HttpServletRequest httpRequest = (HttpServletRequest) WebUtils
					.getRequest(key);
			HttpServletResponse httpResponse = (HttpServletResponse) WebUtils
					.getResponse(key);
			...
			if (StringUtils.isNotBlank(sid)) {
				...
				httpRequest.setAttribute(
						ShiroHttpServletRequest.REFERENCED_SESSION_ID, sid);
				httpRequest.setAttribute(
			...
			if (referer != null
					&& referer.contains("/login.html?forward")) {
