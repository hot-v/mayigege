...
abstract public class BaseController {
	...
	public void handleException(Exception exception, HttpServletRequest request, HttpServletResponse response) {
		...
		if(requestedWith == null || !"XMLHttpRequest".equals(requestedWith)) {
		   ...
		   /* RequestDispatcher rd =  request.getSession().getServletContext().getRequestDispatcher("/WEB-INF/views/500.ftl"); 
		    try {
	...
	public SysUser getCurrentUser(){
		SysUser sysUser = (SysUser)SecurityUtils.getSubject().getSession().getAttribute(AppContextHolder.USER_INFO);
		return sysUser;
	}
}
