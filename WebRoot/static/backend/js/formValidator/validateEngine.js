	...
	for (var name in errors) {
			if ((typeof errors[name]) != "string") {
				for(var i = 0; i < errors[name].length; i++) {
					$("#errormsg-"+errors[name][i],this).text(name);
					$("#errormsg-"+errors[name][i],this).addClass("errMsgBd");
					
					$("[name='"+errors[name][i]+"']:first",this).focus(function(){
						$("#errormsg-"+$(this).attr("name")).text('');
						$("#errormsg-"+$(this).attr("name")).removeClass("errMsgBd");
					});
				}
			} else {
				$("#errormsg-"+errors[name],this).text(name);
				$("#errormsg-"+errors[name],this).addClass("errMsgBd");
				
				$("[name='"+errors[name]+"']:first",this).focus(function(){
					$("#errormsg-"+$(this).attr("name")).text('');
					$("#errormsg-"+$(this).attr("name")).removeClass("errMsgBd");
				});
			}
	}
	if ($('span.errMsgBd',this).length == 0) {
		return true;
	}
	var n = noty({
        text        : "表单验证出错",
...
function notRegex(config, parameters, value) {
	if (isBlank(value)) return true;
	var a = RegExp(config);
	return !a.test(value);
}
			...
			if (RegExp("^"+name+"$").test(paramMap[i])) {
				...
				for ( var j = 0; j < config[name].length; j++) {
					...
					if (typeof(validateConfig.rule) == "function"){
						...
						var flag = {code:0,msg:"验证异常"};
						var data = {rule_value:val,rule:validateConfig.rule,rule_name:paramMap[i]};
						for(var ii = 0; ii < paramMap.length;ii++){
							data[paramMap[ii]] = map.get(paramMap[ii]);
						}
						...
						if(flag.code == 0) {
							if (flag.errors)
							{
								for(var inputName in flag.errors)
								{
									putError(result, inputName, flag.errors[inputName]);
								}
							} else if (flag.msg && flag.msg != "")
								putError(result, paramMap[i], flag.msg);
							else
								putError(result, paramMap[i], validateConfig.msg);
							break;
						}
